  ...
  return <div className="auth-screen">
    ...
    <div className="auth-panel"><div className="auth-form-box">
      ...
      <form onSubmit={handleSubmit}><div style={{display:'flex', flexDirection:'column', gap:16}}>
        ...
        <div className="form-group"><div style={{display:'flex',justifyContent:'space-between'}}><label className="form-label" htmlFor="senha">Senha</label><span style={{fontSize:13,color:'var(--text-3)'}}>Recuperação será migrada depois</span></div><div className="input-icon-wrap"><i className="fa-solid fa-lock input-icon"/><input id="senha" className="form-input" type={showPassword?'text':'password'} required placeholder="••••••••" value={form.senha} onChange={(e)=>setForm({...form,senha:e.target.value})}/><button type="button" className="input-eye" onClick={()=>setShowPassword(!showPassword)}><i className={`fa-solid ${showPassword?'fa-eye-slash':'fa-eye'}`}/></button></div></div>
