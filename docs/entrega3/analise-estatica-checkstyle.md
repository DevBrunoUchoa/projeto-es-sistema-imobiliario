...
Java, via Maven (`mvn checkstyle:checkstyle`). Medição de 19/07/2026.

**2.435 violações** em **154 arquivos**. A lógica de negócio não é afetada — a
...
|---|---|---|
| `JavadocVariableCheck` | 656 | falta Javadoc em atributo |
| `LineLengthCheck` | 467 | linha > 80 colunas |
| `FinalParametersCheck` | 419 | parâmetro sem `final` |
...
| `OperatorWrapCheck` | 19 | quebra de linha em operador |
| `RedundantModifierCheck` | 12 | modificador redundante |
| `LeftCurlyCheck` | 8 | posição da chave de abertura |

## Natureza das ocorrências

Agrupando por natureza, ~43% são **documentação** (`Javadoc*`), ~18% **estilo de
parâmetro** (`FinalParameters`), ~19% **formatação** (`LineLength`,
